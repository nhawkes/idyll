...
fn each_renders_fixed_rows_including_nested_fragments() {
    ...
    assert!(
        html.contains("<li>x</li>") && html.contains("<li>y</li>"),
        "tail rows lost: {html}"
    );
    ...
    assert!(
        html.contains("<li>a</li>") && html.contains("<li>b</li>"),
        "nested rows lost: {html}"
    );
    assert!(
        html.contains("<li>intro</li>"),
        "untitled section's rows lost: {html}"
    );
    // The empty-title section renders no h2 (the @if's else is empty).
    assert_eq!(
        html.matches("<h2>").count(),
        2,
        "unexpected heading count: {html}"
    );
}
...
mod live {
    ...
    use idyll::{live_view, Ctx, MutableSignal, Setup};
    ...
    async fn bars(ctx: Ctx<Setup, Msg>) -> idyll::Result {
        let cols: Vec<MutableSignal<String>> = (0..3)
            .map(|_| ctx.mutable_signal("h0".to_string()))
            .collect();
    ...
    fn fixed_rows_keep_their_reactive_bindings_alive() {
        ...
        let styles: Vec<&str> = commands
            ...
            .filter_map(|c| match c {
                DomCommand::SetAttr { name, value, .. } if name == "style" => Some(value.as_str()),
                _ => None,
            })
            .collect();
        assert_eq!(
